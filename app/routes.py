...
def user(username):
    ...
    page = request.args.get('page', 1, type=int)
    # query = user.posts[::-1]  
    query = Post.query.filter_by(user_id=user.id).order_by(Post.created_at.desc())
...
def unfollow(username):
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            return redirect(url_for('index'))
        if user == current_user:
            flash('You cannot ufollow yourself!')
            return redirect(url_for('user', username=username))
...
def explore():
    ...
    # posts = db.paginate(post_list, page, per_page=app.config['POSTS_PER_PAGE'], error_out=False)
